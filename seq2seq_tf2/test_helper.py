...
def beam_decode(model, batch, vocab, params):
    ...
    def decode_onestep(dec_input, dec_state, enc_output):#enc_inp, enc_outputs, dec_input, dec_state, enc_extended_inp, batch_oov_len, enc_pad_mask, use_coverage, prev_coverage):
        """
        ...
        """
        # enc_output, dec_inp, dec_hidden, dec_tar - model.call params - return predictions, dec_hidden
        ...
        context_vector, attentions = model.attention(dec_states, enc_output)
        
        _, final_dists, dec_hidden = model.decoder(dec_input, None, None, context_vector)
    ...
    while steps < params['max_dec_steps'] and len(results) < params['beam_size']:
        ...
        returns = decode_onestep(dec_input, dec_states, enc_outputs)
